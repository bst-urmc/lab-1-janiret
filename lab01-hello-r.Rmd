...
date: "2022-09-13 (updated `r Sys.Date()` by JNM)"
author: "Janiret Narvaez Miranda"
output: 
  html_document: 
    self_contained: no
    fig_height: 9
...
ggplot(data = dino_data, mapping = aes(x = x, y = y, color= "red")) +
  geom_point() +
...
```{marginfigure}
JNM Answer: The correlation value increases compared to the dino.*
```
...
```{marginfigure}
JNM Answer: compared to "dino" the correlation seems to decrease. 
```
